...
const Feedback2: React.FC = () => {
  ...
  const sliderArray = [...firstHalf, ...sliderArrayIndex, ...secondHalf];
  console.log(secondHalf,firstHalf,sliderArray);
  ...
  console.log(Math.floor((slidesPerPage/2)-1));
  ...
  const nextSlide = () => {
    ...
    setCurrentSlide(
      currentSlide <= OurFeedbacks.length-1 + Math.floor((slidesPerPage/2)-1)
        ? currentSlide + 1
        : 0
    );
  };
  ...
  const animateSlide = () => {
    ...
    // }
let animation:any;
    if(currentSlide <= Math.floor((slidesPerPage/2)-1)){
 animation= SliderConRef.current?.animate(
            [
              {
                transform: `translateX(-${currentSlide * currentPagWidth}px)`,
              },
    ...
    }else if (currentSlide >= Math.floor(slidesPerPage / 2 - 1)){
    ...
    animation = SliderConRef.current?.animate(
      ...
      currentSlide ===
        OurFeedbacks.length - 2 + Math.floor(slidesPerPage / 2 - 1) &&
      SliderConRef.current &&
      currentSlide >= Math.floor(slidesPerPage / 2 - 1)
    ) {
      SliderConRef.current.style.display = `none`;
      //   SliderConRef.current.style.transform = `translateX(-${
      //     2 * currentPagWidth
      //   })`;
      setTimeout(() => {
        if (SliderConRef.current)
          //   SliderConRef.current.style.display = `flex`;
          "";
      }, 3000);

      SliderConRef.current.style.transition = "0s";
    }
      // const animation =
      //   currentSlide <= Math.floor((slidesPerPage/2)-1)
    ...
    //   );
            //   if(currentSlide ===
            //       OurFeedbacks.length - 2 + Math.floor(slidesPerPage / 2 - 1) && SliderConRef.current){
    ...
    <div className="bg-[#f7fafd] py-20 w-full flex items-center justify-center">
      <div className="max-width flex items-center justify-center flex-col">
        <div className="flex items-center justify-center flex-col">
          <BigText text="What Users Saying!" textStyles="base:text-2xl" />
          <AnimationWidget />
          <div className="text-center max-w-[500px]">
            <Text
              text="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
              textStyles="base:text-xs base:text-center"
            />
          </div>
        </div>
        <div className="bg-white py-8 mt-8 px-10 rounded flex items-start justify-between max-w-3xl w-full gap-5 relative flex-wrap base:justify-center base:items-center ">
          ...
          <div className="flex items-start justify-center flex-col base:items-center">
            <h2 className="t text-lg text-darker_text base:text-base">
              {currentSlideData.name}
            </h2>
            <p className="text-[#c679e3] font-light text-sm base:text-xs">
              {currentSlideData.department}
            </p>
            <p className="text-[15px] mt-5 text-dark_text font-light base:text-center base:text-[13px]">
